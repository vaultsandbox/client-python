...
_WEBHOOK_NOT_FOUND_PATTERN = re.compile(r"\bwebhook\b.*\b(not found|does not exist)\b", re.IGNORECASE)
...
class ApiClient:
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if response.status_code == 409:
            if _WEBHOOK_LIMIT_PATTERN.search(message):
    ...
    async def get_inbox_webhook(self, email_address: str, webhook_id: str) -> WebhookData:
        ...
        response = await self._request(
            "GET", f"/api/inboxes/{encoded_addr}/webhooks/{encoded_id}"
        )
        return self._parse_webhook_data(response.json())
