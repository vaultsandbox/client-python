...
class TestVaultSandboxClientInitialization:
    ...
    def test_create_strategy_auto_defaults_to_sse(self) -> None:
        """Test _create_strategy returns SSEStrategy for AUTO (lines 249, 252)."""
        from vaultsandbox.strategies import SSEStrategy
        ...
        assert isinstance(strategy, SSEStrategy)
...
class TestImportValidationErrors:
    ...
    async def test_import_invalid_keypair(self) -> None:
        ...
        with patch("vaultsandbox.client.validate_keypair", return_value=False):
            with pytest.raises(InvalidImportDataError, match="Invalid keypair"):
                await client.import_inbox(
                    ExportedInbox(
                        version=1,
                        email_address="test@example.com",
